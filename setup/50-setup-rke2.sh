...
case $1 in
    "cp")
        cat $CURDIR/51-setup-rke2-controlplane.sh | $QSSH $2 sudo NOKUBEPROXY=$NOKUBEPROXY bash
    ...
    "worker")
        ...
        sed 's/{{CONTROLPLANE_IP}}/'$3'/' $CURDIR/52-setup-rke2-worker.sh | $QSSH $2 sudo NOKUBEPROXY=$NOKUBEPROXY bash
        ;;
    *)
